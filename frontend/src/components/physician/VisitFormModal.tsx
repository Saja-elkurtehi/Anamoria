...
import { demoPatient, DEMO_PATIENT_ID, LAST_APPOINTMENT } from '../../data/mockData';
...
export default function VisitFormModal({ nodes, onClose, onAddedToTimeline }: Props) {
  const { state, addTimelineNode, updateTimelineNode } = useApp();

  const recentNodes    = nodes.filter(n => n.eventDate > LAST_APPOINTMENT);
  ...
  const prefillSinceLast = [
    ...
    ...state.documents.map(d => `• Patient uploaded ${d.fileName}.`),
    ...state.missingInfo
  ...
  const prefillObjective = [
    ...
    state.documents.length > 0
      ? `Uploaded documents reviewed: ${state.documents.map(d => d.fileName).join(', ')}.`
      : null,
  ...
  const [selectedDocIds, setSelectedDocIds] = useState<Set<string>>(
    () => new Set(state.documents.map(d => d.documentId))
  );
  ...
  function handleAddToTimeline() {
    ...
    const linkedDocs  = state.documents.filter(d => selectedDocIds.has(d.documentId));

    const node: TimelineNode = {
      nodeId:            `node-visit-${Date.now()}`,
      patientId:         DEMO_PATIENT_ID,
      type:              'PHYSICIAN_ENTERED',
      title:             `Visit note — Dr. Amir Khan (${new Date().toLocaleDateString('en-CA', { month: 'long', day: 'numeric', year: 'numeric' })})`,
    ...
    if (markReviewed) {
      linkedNodes
        .filter(n => n.type === 'PATIENT_SYMPTOM' && n.verificationStatus === 'NEEDS_REVIEW')
        .forEach(n => updateTimelineNode(n.nodeId, { verificationStatus: 'REVIEWED_NOT_VERIFIED' }));
    }
            ...
            <div className="p-6 space-y-7">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {[
                    { label: 'Patient',           value: demoPatient.name,                                     red: false },
                    { label: 'Age / Sex',          value: `${demoPatient.age} y/o · ${demoPatient.gender}`,    red: false },
                    { label: 'Last appointment',  value: demoPatient.lastAppointment,                          red: false },
                    { label: 'Known conditions',  value: demoPatient.conditions.join(', '),                    red: false },
                    { label: '⚠ Allergies',       value: demoPatient.allergies.join(', '),                     red: true  },
                    { label: 'Current medications', value: demoPatient.medications.map(m => m.name).join(', '), red: false },
                  ].map(({ label, value, red }) => (
                    <div key={label}>
              ...
              <EditableArea
                ...
                rows={recentSymptoms.length + state.documents.length + 2}
              ...
              <ClinicalArea
                ...
                placeholder="Physician to complete assessment / clinical impression. Suggested context from timeline is in the sections above."
                rows={4}
              />
              ...
              <div>
                <SectionHeader
                  ...
                  source={`${selectedDocIds.size} of ${state.documents.length} selected`}
                  color="blue"
                />
                {state.documents.length === 0 ? (
                  <div className="border border-gray-200 rounded-xl px-4 py-6 text-center">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <span className="text-xs text-gray-600">
                      Mark selected patient-reported symptoms as reviewed
                    </span>
                  </label>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-5 py-2 text-sm font-semibold bg-violet-600 text-white rounded-xl hover:bg-violet-700 active:bg-violet-800 transition-colors flex items-center gap-2"
                    >
                      <Stethoscope className="w-4 h-4" />
                      Add visit note to timeline
                    </button>
                  </div>
                </div>
